...
- **Revisión crítica**: el asistente fue instruido para cuestionar decisiones (no solo ejecutar pedidos) y documentar el porqué de cada cambio de rumbo — visible en las secciones "Decisión superada" del ADR. Como doble check, ya con el proyecto completo, se hizo una auditoría post-entrega actuando como evaluador senior (no instructor) para verificar que los hitos prometidos en el ADR se cumplieran de verdad en el código: de ahí salieron 5 correcciones reales (§20-§24 del ADR) y la cobertura completa de la colección de Postman, verificada con Newman en vez de solo importada.
